(function(sandbox) {
  ...
  sandbox.init = function() {
    ...
    this.api = Instajam.init({
      clientId: 'ab9ad048ea114512a095a0c3237fc5ca',
      redirectUri: 'https://www.misiva.com.ec/vespainstagram/instajam/',
  ...
  sandbox.test = function(method) {
    ...
    case 'user.get':
      this.api.user.get('vespa_ecuador', function(data) {
        showResults(data);
      });
      break;

    case 'user.media':
      this.api.user.media('vespa_ecuador', function(data) {
        showResults(data);
      });
      break;
